...
def write_job(Job, Version=-1, SkipEvents=0, MaxEvents=-1, NFile=None, FileSplit=-1, workdir="workdir", LumiWeight=1):
    ...
    for cycle in Job.Job_Cylce:
        ...
        tempChild.setAttribute(
            'OutputDirectory', cycle.OutputDirectory + '/' + workdir + '/')
        ...
        for p in range(len(cycle.Cycle_InputData)):
            ...
            InputGrandchild.setAttribute('Lumi', str(
                float(cycle.Cycle_InputData[p].Lumi) * cycleLumiWeight))
            InputGrandchild.setAttribute('Type', cycle.Cycle_InputData[p].Type)
            InputGrandchild.setAttribute(
                'Version', cycle.Cycle_InputData[p].Version)
            if FileSplit != -1:
                InputGrandchild.setAttribute('Cacheable', 'False')
            else:
                InputGrandchild.setAttribute(
                    'Cacheable', cycle.Cycle_InputData[p].Cacheable)
            ...
            Datachild = doc.createElement(
                cycle.Cycle_InputData[p].io_list.InputTree[0])
            InputGrandchild.appendChild(Datachild)
    ...
    def __init__(self, xmlfile):
        ...
        while '<JobConfiguration' not in line:
            ...
            if 'ConfigParse' in line:
                self.ConfigParse = parseString(
                    line).getElementsByTagName('ConfigParse')[0]
                self.NEventsBreak = int(
                    self.ConfigParse.attributes['NEventsBreak'].value)
                self.FileSplit = int(
                    self.ConfigParse.attributes['FileSplit'].value)
                if self.ConfigParse.hasAttribute('AutoResubmit'):
                    self.AutoResubmit = int(
                        self.ConfigParse.attributes['AutoResubmit'].value)
                if self.ConfigParse.hasAttribute('MaxJobsPerProcess'):
                    self.MaxJobsPerProcess = int(
                        self.ConfigParse.attributes['MaxJobsPerProcess'].value)
                if self.ConfigParse.hasAttribute('RemoveEmptyFileSplit'):
                    self.RemoveEmptyFileSplit = bool(
                        self.ConfigParse.attributes['RemoveEmptyFileSplit'].value)

            if 'ConfigSGE' in line:
                self.ConfigSGE = parseString(
                    line).getElementsByTagName('ConfigSGE')[0]
                self.RAM = self.ConfigSGE.attributes['RAM'].value
                self.WALLTIME = self.ConfigSGE.attributes['WALLTIME'].value
...
def get_number_of_events(Job, Version, atleastOneEvent=False):
    InputData = filter(lambda inp: inp.Version ==
                       Version[0], Job.Job_Cylce[0].Cycle_InputData)[0]
    NEvents = 0
    ...
    for entry in InputData.io_list.FileInfoList[:]:
        for name in entry:
            if name.endswith('.root'):
                ...
                try:
                    n = f.Get(
                        str(InputData.io_list.InputTree[2])).GetEntriesFast()
                    if n < 1:
    ...
    if NEventsBreak != 0 and FileSplit <= 0:
        ...
        for i in xrange(NFiles):
            ...
            for line in header.header:
                outfile.write(line)
            outfile.write(write_job(Job, Version, i * SkipEvents,
                                    MaxEvents, i, -1, workdir, LumiWeight))
            outfile.close()

    elif FileSplit > 0:
        for entry in Version:
            NEvents = get_number_of_events(
                Job, [entry], not FileSplitCompleteRemove)
            if NEvents <= 0:
            ...
            for cycle in Job.Job_Cylce:
                for p in xrange(len(cycle.Cycle_InputData)):
                    if(cycle.Cycle_InputData[p].Version == entry) or Version == -1:
                        Total_xml = len(
                            cycle.Cycle_InputData[p].io_list.FileInfoList)
                        numberOfJobs = int(
                            math.ceil(float(Total_xml) / FileSplit))
                        numberOfSplits = FileSplit
                        if numberOfJobs > MaxJobs and MaxJobs > 0:
                            numberOfSplits = int(
                                math.ceil(float(Total_xml) / MaxJobs))
                            numberOfJobs = int(
                                math.ceil(float(Total_xml) / numberOfSplits))
                        ...
                        for it in range(numberOfJobs):
                            outfile = open(
                                path + '_' + str(it + 1) + '.xml', 'w+')
                            for line in header.header:
                                outfile.write(line)
                            outfile.write(
                                write_job(Job, Version, 0, -1, it, numberOfSplits, workdir))
                            outfile.close()
                            NFiles += 1
    else:
        NFiles += 1
...
def result_info(Job, path, header, other=[]):
    ...
    for cycle in ResultJob.Job_Cylce:
        for inputdata in cycle.Cycle_InputData:
            ...
            inputdata.io_list.FileInfoList = [['In', 'Lumi', inputdata.io_list.FileInfoList[0][2], 'FileName',
                                               cycle.OutputDirectory + "/" + path + "/uhh2.AnalysisModuleRunner.*." + inputdata.Version + "_*.root"]]
            inputdata.io_list.InputTree = [
                'InputTree', 'Name', inputdata.io_list.other[other_index][2]]
            if not other:
                ...
                if len(other) == 1:
                    if other[0] == "-1":
                        inputdata.io_list.other = [
                            ['OutputTree', 'Name', inputdata.io_list.other[other_index][2]]]
                    else:
                        inputdata.io_list.other = [
                            ['OutputTree', 'Name', other[0]]]
                else:
