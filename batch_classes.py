...
def submit_qsub(NFiles, Stream, name, workdir):
    ...
    proc_qstat = Popen(['condor_submit' + ' ' + workdir + '/CondorSubmitfile_' + name + '.submit' +
                        ' -a "Stream=' + Stream.split('/')[1] + '" -a "queue ' + str(NFiles) + '"'], shell=True, stdout=PIPE)
...
def resubmit(Stream, name, workdir, header):
    ...
    proc_qstat = Popen(['condor_submit' + ' ' + workdir + '/CondorSubmitfile_' + name +
                        '.submit' + ' -a "Stream=' + Stream.split('/')[1] + '"'], shell=True, stdout=PIPE)
    return (proc_qstat.communicate()[0].split()[7]).split('.')[0]
...
def add_histos(directory, name, NFiles, workdir, outputTree, onlyhists, outputdir):
    ...
    proc = None
    position = -1
    # -v 1 ' # the -v stopped working in root 6.06/01 now we get a lot of crap
    ...
    source_files = ""
    if NFiles > 1:
        source_files = directory + workdir + \
            '/' + name + '_{' + string + '}.root'
    else:
    ...
    if not string.isspace():
        proc = Popen([str(command_string + directory + name + '.root ' + source_files +
                          ' > ' + outputdir + '/hadd.log')], shell=True, stdout=FNULL, stderr=FNULL)
    else:
